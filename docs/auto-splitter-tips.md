...
If you try to `print()` a table, you will receive a not-very-useful `table: 0x123456`. This utility inspects a table for you and pretty-prints it with some indentation.

### `utils.shallow_copy_table`
...
The `old.something=current.something` calls can all be replaced by `utils.shallow_copy_table`, as long as your tables are on a single level:

```.lua
old = utils.shallow_copy_table(current)
current.level = readAddress(...)
current.minutes = readAddress(...)
-- ...
```
